...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gjang is "gjang". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gjang", keep that.
	if name == "gjang" || name == "gjang-testnet" {
		name = "Gjang"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gjang" instances.
var isOldGjangResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gjang 1.4 are used if they exist.
	if warn, isOld := isOldGjangResource[path]; isOld {
		oldpath := ""
		if c.name() == "gjang" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGjangResourceWarning, "Using deprecated resource file %s, please move this file to the 'gjang' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-jjang-keystore")
		isEphemeral = true
	}
