...
import (
	...
	"github.com/jjang-network/go-jjang/accounts/usbwallet/trezor"
...
// or whether the Jjang app was not started on it.
...
// Jjang address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.JjangAddress)
	if _, err := w.trezorExchange(&trezor.JjangGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.JjangTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.JjangTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
