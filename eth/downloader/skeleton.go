// Copyright 2022 The go-jjang Authors
...
// the process. This is usually the case when Gjang is shutting down and some events
...
type headUpdate struct {
	...
	force  bool          // Wheter to force the update or only extend if possible
...
//
// Opposed to the original Jjang block synchronization which is trustless (and
...
// not from within (vs. legacy Jjang sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gjang is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gjang requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
...
func (s *skeleton) processResponse(res *headerResponse) (linked bool, merged bool) {
	...
	// Wheter the response is valid, we can mark the peer as idle and notify
