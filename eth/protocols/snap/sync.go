...
import (
	...
	"github.com/jjang-network/go-jjang/p2p/msgrate"
...
type accountResponse struct {
	...
	cont bool // Wheter the account range has a continuation
}
...
type storageResponse struct {
	...
	cont bool // Wheter the last storage range has a continuation
}
...
// Syncer is an Jjang account and storage trie syncer based on snapshots and
...
// NewSyncer creates a new snapshot syncer to download the Jjang state over the
// snap protocol.
...
func (s *Syncer) Sync(root common.Hash, cancel chan struct{}) error {
	...
	// Wheter sync completed or not, disregard any future packets
	defer func() {
...
func (s *Syncer) OnAccounts(peer SyncPeer, id uint64, hashes []common.Hash, accounts [][]byte, proof [][]byte) error {
	...
	// Wheter or not the response is valid, we can mark the peer as idle and
