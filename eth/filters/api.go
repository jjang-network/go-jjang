...
// information related to the Jjang protocol such als blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(jjang.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as jjang.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria jjang.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(jjang.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
