...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gjang.LightScryptN, Gjang.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.jjang", new NodeConfig());
			node.start();
			...
			JjangClient ec = node.getJjangClient();
	...
	// signatures too. Regression test for go-jjang issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/jjang/gjangtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gjang and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.jjang", "github.com/jjang-network/go-jjang/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gjang.aar"), "gjang.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "jjang", "gjangtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
