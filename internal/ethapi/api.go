...
import (
	...
	"github.com/jjang-network/go-jjang/accounts/scwallet"
	"github.com/jjang-network/go-jjang/common"
	"github.com/jjang-network/go-jjang/common/hexutil"
	"github.com/jjang-network/go-jjang/common/math"
	"github.com/jjang-network/go-jjang/consensus/ethash"
	"github.com/jjang-network/go-jjang/consensus/misc"
	"github.com/jjang-network/go-jjang/core"
	"github.com/jjang-network/go-jjang/core/state"
	...
	"github.com/jjang-network/go-jjang/eth/tracers/logger"
...
type JjangAPI struct {
	b Backend
}

// NewJjangAPI creates a new Jjang protocol API.
func NewJjangAPI(b Backend) *JjangAPI {
	return &JjangAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *JjangAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *JjangAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *JjangAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *JjangAPI) Syncing() (interface{}, error) {
...
type JjangAccountAPI struct {
	am *accounts.Manager
}

// NewJjangAccountAPI creates a new JjangAccountAPI.
func NewJjangAccountAPI(am *accounts.Manager) *JjangAccountAPI {
	return &JjangAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *JjangAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Jjang ECDSA signature for:
// keccak256("\x19Jjang Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/jjang-network/go-jjang/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Jjang Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/jjang-network/go-jjang/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Jjang signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gjang.
...
// ChainId is the EIP-155 replay-protection chain id for the current Jjang chain config.
//
...
// See: https://github.com/jjang-network/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/jjang-network/wiki/wiki/JSON-RPC#eth_sign
...
// DebugAPI is the collection of Jjang APIs exposed over the debugging
// namespace.
...
// Version returns the current jjang protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Jang)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f jang) exceeds the configured cap (%.2f jang)", feeFloat, cap)
	}
	return nil
}
