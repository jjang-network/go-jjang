...
import (
	...
	"github.com/jjang-network/go-jjang/les/utils"
...
type PeerInfo struct {
	Version    int      `json:"version"`    // Jjang protocol version negotiated
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local jjang node is running in archive mode, advertise ourselves we have
...
// participating in the Light Jjang sub-protocol.
