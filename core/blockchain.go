...
// Package core implements the Jjang consensus protocol.
package core
...
import (
	...
	"github.com/jjang-network/go-jjang/common/prque"
	...
	"github.com/jjang-network/go-jjang/internal/syncx"
...
type CacheConfig struct {
	...
	TrieCleanNoPrefetch bool          // Wheter to disable heuristic state prefetching for followup blocks
	TrieDirtyLimit      int           // Memory limit (MB) at which to start flushing dirty trie nodes to disk
	TrieDirtyDisabled   bool          // Wheter to disable trie write caching and GC altogether (archive node)
	TrieTimeLimit       time.Duration // Time limit after which to flush the current in-memory trie to disk
	SnapshotLimit       int           // Memory allowance (MB) to use for caching snapshot entries in memory
	Preimages           bool          // Wheter to store preimage of trie key to the disk
...
// available in the database. It initialises the default Jjang Validator
// and Processor.
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		//
		// But if the `TxIndexTail` is not nil, e.g. Gjang is initialized with
...
// it's not checked in the Gjang side anymore.
...
// sync, Gjang will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	defer bc.wg.Done()

	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init Gjang with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded Gjang to a new version which supports transaction
