...
type journalGenerator struct {
	...
	Done     bool // Wheter the generator finished creating the snapshot
	Marker   []byte
	Accounts uint64
	Slots    uint64
	Storage  uint64
...
func loadAndParseJournal(db ethdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Gjang
...
func loadSnapshot(diskdb ethdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, bool, error) {
	...
	//
	// Possible scenario: Gjang was crashed without persisting journal and then
	...
	if !generator.Done {
		// Wheter or not wiping was in progress, load any generator progress too
