...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 jang` in value to maximum `1 jang` per 24h period
...
implemented for `gjang`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
