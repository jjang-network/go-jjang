// Copyright 2017 The go-jjang Authors
// This file is part of go-jjang.
//
// go-jjang is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// go-jjang is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with go-jjang. If not, see <http://www.gnu.org/licenses/>.

// faucet is an Jang faucet backed by a light client.
package main
...
import (
	...
	"github.com/jjang-network/go-jjang/accounts/keystore"
	"github.com/jjang-network/go-jjang/cmd/utils"
	...
	"github.com/jjang-network/go-jjang/ethstats"
	"github.com/jjang-network/go-jjang/les"
	...
	"github.com/jjang-network/go-jjang/p2p/nat"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Jjang protocol")
	statsFlag   = flag.String("ethstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Jangs to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Jjang and the faucet")
...
var (
	jang = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Jangs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Jjang address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Jjang light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Jjang protocol stack
	client *ethclient.Client   // Client connection to the Jjang chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Jjang light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Jjang service: %w", err)
	}
...
// close terminates the Jjang connection and tears down the faucet.
...
// apiHandler handles requests for Jang grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err = send(wsconn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, jang),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Jjang address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/jjang-network/go-jjang/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
		...
		if timeout = f.timeouts[id]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), jang)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, jang)
			peers := f.stack.Server().PeerCount()
...
// the uniqueness identifier (user id/username), username, avatar URL and Jjang address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Jjang address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Jjang address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Jjang address to fund on
// success.
...
// returning the username, avatar URL and Jjang address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Jjang address and profile URL.
	//
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No Jjang address found to fund. Please check the post URL and verify that it can be viewed publicly.")
	}
	var avatar string
...
// authNoAuth tries to interpret a faucet request as a plain Jjang address,
