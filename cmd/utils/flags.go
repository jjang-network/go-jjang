...
// Package utils contains internal helper functions for go-jjang commands.
package utils
...
import (
	...
	"github.com/jjang-network/go-jjang/common/fdlimit"
	"github.com/jjang-network/go-jjang/consensus"
	"github.com/jjang-network/go-jjang/consensus/ethash"
	...
	"github.com/jjang-network/go-jjang/crypto"
	"github.com/jjang-network/go-jjang/eth"
	ethcatalyst "github.com/jjang-network/go-jjang/eth/catalyst"
	...
	"github.com/jjang-network/go-jjang/eth/tracers"
	"github.com/jjang-network/go-jjang/ethdb"
	"github.com/jjang-network/go-jjang/ethdb/remotedb"
	"github.com/jjang-network/go-jjang/ethstats"
	"github.com/jjang-network/go-jjang/graphql"
	"github.com/jjang-network/go-jjang/internal/ethapi"
	"github.com/jjang-network/go-jjang/internal/flags"
	"github.com/jjang-network/go-jjang/les"
	lescatalyst "github.com/jjang-network/go-jjang/les/catalyst"
	"github.com/jjang-network/go-jjang/log"
	"github.com/jjang-network/go-jjang/metrics"
	"github.com/jjang-network/go-jjang/metrics/exp"
	"github.com/jjang-network/go-jjang/metrics/influxdb"
	...
	"github.com/jjang-network/go-jjang/p2p/netutil"
...
var (
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in jang) that can be sent via the RPC APIs (0 = no cap)",
	...
	IgnoreLegacyReceiptsFlag = &cli.BoolFlag{
		...
		Usage:    "Gjang will start up even if there are legacy receipts in freezer",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(RopstenFlag.Name) {
			// Maintain compatibility with older Gjang configurations storing the
...
// for Gjang and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gjang account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Jjang client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Jjang) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Jjang service: %v", err)
		}
...
// RegisterEthStatsService configures the Jjang Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Jjang Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gjang.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gjang.", tagsMap)
		}
