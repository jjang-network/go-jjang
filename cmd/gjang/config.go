...
import (
	...
	"github.com/jjang-network/go-jjang/accounts/external"
	"github.com/jjang-network/go-jjang/accounts/keystore"
	"github.com/jjang-network/go-jjang/accounts/scwallet"
	"github.com/jjang-network/go-jjang/accounts/usbwallet"
...
func loadConfig(file string, cfg *gjangConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gjang.ipc"
	return cfg
}

// makeConfigNode loads gjang configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gjangConfig) {
	// Load defaults.
	cfg := gjangConfig{
...
// makeFullNode loads gjang configuration and creates the Jjang backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `gjang db freezer-migrate`.")
		}
	}
	...
	// Add the Jjang Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gjangConfig) {
