...
`
	...
	{
		gjang := runGjang(t, "account", "list", "--datadir", datadir)
		gjang.Expect(want)
		gjang.ExpectExit()
	}
	{
		gjang := runGjang(t, "--datadir", datadir, "account", "list")
		gjang.Expect(want)
		gjang.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gjang := runGjang(t, "account", "new", "--lightkdf")
	defer gjang.ExpectExit()
	gjang.Expect(`
...
func TestAccountHelp(t *testing.T) {
	gjang := runGjang(t, "account", "-h")
	gjang.WaitExit()
	if have, want := gjang.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gjang = runGjang(t, "account", "import", "-h")
	gjang.WaitExit()
	if have, want := gjang.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gjang := runGjang(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gjang.ExpectExit()
	gjang.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gjang := runGjang(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gjang := runGjang(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gjang.ExpectExit()
	gjang.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gjang.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gjang := runMinimalGjang(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gjang.StderrText(), m) {
...
// https://github.com/jjang-network/go-jjang/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gjang.SetTemplateFunc("keypath", func(file string) string {
