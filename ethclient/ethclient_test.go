...
// Verify that Client implements the jjang interfaces.
var (
	_ = jjang.ChainReader(&Client{})
	_ = jjang.TransactionReader(&Client{})
	_ = jjang.ChainStateReader(&Client{})
	_ = jjang.ChainSyncReader(&Client{})
	_ = jjang.ContractCaller(&Client{})
	_ = jjang.GasEstimator(&Client{})
	_ = jjang.GasPricer(&Client{})
	_ = jjang.LogFilterer(&Client{})
	_ = jjang.PendingStateReader(&Client{})
	// _ = jjang.PendingStateEventer(&Client{})
	_ = jjang.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Jjang Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new jjang service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == jjang.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != jjang.NotFound {
		t.Fatal("error should be jjang.NotFound")
	}
}
