...
// Package gjangclient provides an RPC client for gjang-specific APIs.
package gjangclient
...
// Client is a wrapper around rpc.Client that implements gjang-specific functionality.
//
// If you want to use the standardized Jjang RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg jjang.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg jjang.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gjang node.
...
// MemStats retrieves the current memory stats from a gjang node.
...
// GetNodeInfo retrieves the node info of a gjang node.
