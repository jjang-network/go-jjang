...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Jjang address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Jjang account at a particular block.
    type Account {
    ...
    # Transaction is an Jjang transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Jjang account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Jjang account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Jjang block by number or by hash. If neither is
