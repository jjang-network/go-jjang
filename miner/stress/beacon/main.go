...
import (
	...
	"github.com/jjang-network/go-jjang/core/beacon"
...
type ethNode struct {
	...
	ethBackend *eth.Jjang
	lapi       *lescatalyst.ConsensusAPI
	lesBackend *les.LightJjang
}
...
func makeFullNode(genesis *core.Genesis) (*node.Node, *eth.Jjang, *ethcatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Jjang node
	...
	// Create the node and configure a full Jjang node on it
...
func makeLightNode(genesis *core.Genesis) (*node.Node, *les.LightJjang, *lescatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Jjang node
	datadir, _ := os.MkdirTemp("", "")
